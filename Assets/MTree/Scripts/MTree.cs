                        ...
                        , float gravityStrength, float flatten, float minRadius)
        {
            ...
            while (extremities.Count > 0)
            {
                ...
                while (extremities.Count > 0)
                {
                    ...
                    Queue<Node> children = extremity.Grow(branchLength, extremitySplitProba, maxSplits, rad, splitRadius, splitAngle, creator, randomness, upAttraction, treeTransform, gravityStrength, flatten);
                    if (rad == minRadius)
                        continue;
                    foreach (Node child in children)
                    {
